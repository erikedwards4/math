...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Random function: lognormal_distribution.\n";
    descr += "Outputs tensor of floats from a log-normal distribution, \n";
    descr += "with mean param m and std dev param d>0.\n";
    descr += "\n";
    descr += "For complex output, real/imag parts are set separately.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ lognormal -m-2 -d2.5 -r2 -c3 -o Y \n";
    descr += "$ lognormal -m-2 -d2.5 -r2 -c3 > Y \n";
    descr += "$ lognormal -m-2 -d2.5 -r2 -c3 -t102 > Y \n";
    ...
    void *argtable[] = {a_mn, a_std, a_nr, a_nc, a_ns, a_nh, a_otyp, a_ofmt, a_fo, a_help, a_end};
    ...
    if (o1.T==1)
    {
        ...
        lognormal_distribution<float> distr(mn,std);
    ...
    else if (o1.T==2)
    {
        ...
        lognormal_distribution<double> distr(mn,std);
        try { generate_n(begin(Y),o1.N(),[&distr,&mt_eng](){return distr(mt_eng);}); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem during generate" << endl; return 1; }
        if (Y.size()!=o1.N()) { cerr << progstr+": " << __LINE__ << errstr << "unexpected output size" << endl; return 1; }
        if (wo1)
