...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Random function: chi_squared_distribution.\n";
    descr += "Outputs tensor of floats from a chi-squared distribution, \n";
    descr += "with degrees-of-freedom parameter n>0.\n";
    descr += "\n";
    descr += "For complex output, real/imag parts are set separately.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ chi_squared -n5 -r2 -c3 -o Y \n";
    descr += "$ chi_squared -n5 -r2 -c3 > Y \n";
    descr += "$ chi_squared -n5 -r2 -c3 -t102 > Y \n";


    //Argtable
    int nerrs;
    struct arg_dbl    *a_n = arg_dbln("n","dof","<dbl>",0,1,"degrees-of-freedom param [default=1]");
    ...
    void *argtable[] = {a_n, a_nr, a_nc, a_ns, a_nh, a_otyp, a_ofmt, a_fo, a_help, a_end};
    ...
    if (dof<0.0) { cerr << progstr+": " << __LINE__ << errstr << "n must be nonnegative " << endl; return 1; }
    ...
    if (o1.T==1u)
    {
        ...
        chi_squared_distribution<float> distr(dof);
    ...
    else if (o1.T==2u)
    {
        ...
        chi_squared_distribution<double> distr(dof);
        try { generate_n(begin(Y),o1.N(),[&distr,&pcg_eng](){return distr(pcg_eng);}); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem during generate" << endl; return 1; }
        if (Y.size()!=o1.N()) { cerr << progstr+": " << __LINE__ << errstr << "unexpected output size" << endl; return 1; }
        if (wo1)
