...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets harmonic mean for each vector in X along dim.\n";
    ...
    descr += "$ harmean X -o Y \n";
    descr += "$ harmean X > Y \n";
    descr += "$ harmean -d1 X > Y \n";
    descr += "$ cat X | harmean > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (codee::harmean_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::harmean_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (codee::harmean_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (codee::harmean_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
