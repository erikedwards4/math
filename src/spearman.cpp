...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets Spearman's rank correlation coefficient (rho), \n";
    descr += "for each pair of vectors in X1, X2 along dim.\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ spearman X1 X2 -o Y \n";
    descr += "$ spearman -d1 X1 X2 > Y \n";
    descr += "$ cat X1 | spearman -d2 - X2 > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::spearman_s(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i2.R,i2.C,i2.S,i2.H,o1.iscolmajor(),dim))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::spearman_d(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,i2.R,i2.C,i2.S,i2.H,o1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
