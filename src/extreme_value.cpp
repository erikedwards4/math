...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Random function: extreme_value_distribution.\n";
    descr += "Outputs tensor of floats from an extreme-value distribution, \n";
    descr += "a.k.a the log-Weibull distribution, \n";
    descr += "with shape param a>0 and scale param b>0.\n";
    descr += "\n";
    descr += "For complex output, real/imag parts are set separately.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ extreme_value -a2.5 -b2 -r2 -c3 -o Y \n";
    descr += "$ extreme_value -a2.5 -b2 -r2 -c3 > Y \n";
    descr += "$ extreme_value -a2.5 -b2 -r2 -c3 -t102 > Y \n";


    //Argtable
    int nerrs;
    struct arg_dbl    *a_a = arg_dbln("a","a","<dbl>",0,1,"shape parameter [default=1]");
    struct arg_dbl    *a_b = arg_dbln("b","b","<dbl>",0,1,"scale parameter [default=1]");
    ...
    if (a<=0.0) { cerr << progstr+": " << __LINE__ << errstr << "a (alpha) must be positive " << endl; return 1; }
    ...
    if (o1.T==1u)
    {
        ...
        extreme_value_distribution<float> distr(a,b);
    ...
    else if (o1.T==2)
    {
        ...
        extreme_value_distribution<double> distr(a,b);
        try { generate_n(begin(Y),o1.N(),[&distr,&pcg_eng](){return distr(pcg_eng);}); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem during generate" << endl; return 1; }
        if (Y.size()!=o1.N()) { cerr << progstr+": " << __LINE__ << errstr << "unexpected output size" << endl; return 1; }
        if (wo1)
