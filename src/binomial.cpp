...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Random function: binomial_distribution.\n";
    descr += "Outputs RxC matrix (or RxCxS cube) of uints from a negative binomial distribution,\n";
    descr += "with parameters p and t (p is probabiliy of success).\n";
    descr += "\n";
    descr += "Output is nonnegative ints, but any data type can be used.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ binomial -p0.9 -t7 -r2 -c3 -o Y \n";
    descr += "$ binomial -p0.9 -t7 -r2 -c3 > Y \n";
    descr += "$ binomial -p0.9 -t7 -r2 -c3 -t2 > Y \n";


    //Argtable
    int nerrs;
    struct arg_dbl    *a_p = arg_dbln("p","p","<dbl>",0,1,"p probability parameter [default=0.5]");
    struct arg_int    *a_t = arg_intn("t","t","<uint>",0,1,"t tries parameter [default=1]");
    ...
    void *argtable[] = {a_p, a_t, a_nr, a_nc, a_ns, a_nh, a_otyp, a_ofmt, a_fo, a_help, a_end};
    ...
    //Get t
    if (a_t->count==0) { t = 1; }
    else if (a_t->ival[0]<0) { cerr << progstr+": " << __LINE__ << errstr << "t must be nonnegative" << endl; return 1; }
    else { t = uint(a_t->ival[0]); }
