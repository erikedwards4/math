...
int main(int argc, char *argv[])
{
    ...
    descr += "See sorti for the inverse (indices to sorted X to recover X).\n";
    descr += "\n";
    ...
    if (i1.T==1)
    {
        ...
        if (codee::ranks_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::ranks_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (codee::ranks_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (codee::ranks_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
