...
int main(int argc, char *argv[])
{
    ...
    descr += "Linear transformation of each vec in X using matrix A.\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ linear X A -o Y \n";
    descr += "$ linear X A > Y \n";
    descr += "$ cat X | linear - A > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::linear_s(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,Ly,o1.iscolmajor(),dim))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::linear_d(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,Ly,o1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::linear_c(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,Ly,o1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::linear_z(Y,X1,X2,i1.R,i1.C,i1.S,i1.H,Ly,o1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
