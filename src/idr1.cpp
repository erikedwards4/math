...
int main(int argc, char *argv[])
{
    ...
    descr += "such that the IDR (inter-decile range) is from 0 to 1.\n";
    descr += "\n";
    ...
    struct arg_lit    *a_m = arg_litn("m","m1",0,1,"scale to an IDR of -1 to 1 [default is 0 to 1]");
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::idr1_inplace_s(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,m1)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::idr1_inplace_d(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,m1)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
