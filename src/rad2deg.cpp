...
int main(int argc, char *argv[])
{
    ...
    descr += "Converts radians to degrees for each element of X.\n";
    descr += "For each element: y = x*180/pi.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ rad2deg X -o Y \n";
    descr += "$ rad2deg X > Y \n";
    descr += "$ cat X | rad2deg > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (codee::rad2deg_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::rad2deg_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
