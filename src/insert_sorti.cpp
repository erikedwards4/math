...
int main(int argc, char *argv[])
{
    ...
    descr += "That is, the insert_sorti indices would recover X from the sorted X.\n";
    descr += "\n";
    descr += "This uses (my own implementation of) the insertion sort algorithm.\n";
    descr += "This is faster than qsorti if the vectors are short (e.g., L < 128).\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ insert_sorti X -o Y \n";
    descr += "$ insert_sorti -d1 X > Y \n";
    descr += "$ cat X | insert_sorti -d2 > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::insert_sorti_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::insert_sorti_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::insert_sorti_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::insert_sorti_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
