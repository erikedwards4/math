...
int main(int argc, char *argv[])
{
    ...
    if (i1.T==1)
    {
        ...
        if (i1.N()==o1.N())
        {
            if (codee::atan2_inplace_s(X1,X2,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
        ...
        else
        {
            ...
            if (codee::atan2_s(Y,X1,X2,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
    ...
    else if (i1.T==2)
    {
        ...
        if (i1.N()==o1.N())
        {
            if (codee::atan2_inplace_d(X1,X2,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
                try { ofs1.write(reinterpret_cast<char*>(X1),o1.nbytes()); }
        ...
        else
        {
            ...
            if (codee::atan2_d(Y,X1,X2,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
                try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
