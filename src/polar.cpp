...
int main(int argc, char *argv[])
{
    ...
    descr += "Makes complex-valued Y from real-valued magnitude X1 and phase X2.\n";
    descr += "Magnitude is the modulus (abs) and phase is the argument (arg).\n";
    descr += "\n";
    descr += "X1 and X2 must have the same real-valued data type.\n";
    descr += "X1 and X2 must have the same size or broadcast-compatible sizes.\n";
    descr += "\n";
    descr += "Output (Y) has the corresponding complex data type.\n";
    descr += "Output (Y) has size max(R1,R2) x max(C1,C2).\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ polar X1 X2 -o Y \n";
    descr += "$ polar X1 X2 > Y \n";
    descr += "$ cat X2 | polar X1 > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (codee::polar_s(Y,X1,X2,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::polar_d(Y,X1,X2,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
