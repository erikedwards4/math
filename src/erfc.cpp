...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets complementary error-function of element of X.\n";
    descr += "For each element: y = erfc(x).\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ erfc X -o Y \n";
    descr += "$ erfc X > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (codee::erfc_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::erfc_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (codee::erfc_inplace_c(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (codee::erfc_inplace_z(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
