...
int main(int argc, char *argv[])
{
    ...
    descr += "Floors each element of X (rounds toward -Inf).\n";
    descr += "For each element: y = floor(x).\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ floor X -o Y \n";
    descr += "$ floor X > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::floor_inplace_s(X,i1.N()))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::floor_inplace_d(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::floor_inplace_c(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::floor_inplace_z(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
