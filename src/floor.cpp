...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets floor of each element of X.\n";
    ...
    if (i1.T==1)
    {
        ...
        if (openn::floor_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (openn::floor_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (openn::floor_inplace_c(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (openn::floor_inplace_z(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
