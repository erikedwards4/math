...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Random function: normal_distribution.\n";
    descr += "Outputs tensor of floats from a normal (Gaussian) distribution, \n";
    descr += "with mean param m and std dev param d>0.\n";
    descr += "\n";
    descr += "For complex output, real/imag parts are set separately.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ normal -m-2 -d2.5 -r2 -c3 -o Y \n";
    descr += "$ normal -m-2 -d2.5 -r2 -c3 > Y \n";
    descr += "$ normal -m-2 -d2.5 -r2 -c3 -t102 > Y \n";
    ...
    if (o1.T==1u)
    {
        ...
        normal_distribution<float> distr(mn,std);
    ...
    else if (o1.T==2u)
    {
        ...
        normal_distribution<double> distr(mn,std);
        try { generate_n(begin(Y),o1.N(),[&distr,&pcg_eng](){return distr(pcg_eng);}); }
        catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem during generate" << endl; return 1; }
        if (Y.size()!=o1.N()) { cerr << progstr+": " << __LINE__ << errstr << "unexpected output size" << endl; return 1; }
        if (wo1)
