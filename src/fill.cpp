...
int main(int argc, char *argv[])
{
    ...
    descr += "Output Y is a tensor filled with val.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ fill -v1e6 -r2 -c3 -o Y \n";
    descr += "$ fill -v1e6 -r2 -c3 > Y \n";
    descr += "$ fill -v-1e6 -r2 -c3 -s2 -t2 > Y \n";
    ...
    void *argtable[] = {a_v, a_nr, a_nc, a_ns, a_nh, a_otyp, a_ofmt, a_fo, a_help, a_end};
    ...
    if (o1.T==1)
    {
        ...
        if (codee::fill_s(Y,int(o1.N()),float(val)))
    ...
    else if (o1.T==2)
    {
        ...
        if (codee::fill_d(Y,int(o1.N()),double(val)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==101)
    {
        ...
        if (codee::fill_c(Y,int(o1.N()),float(val)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (o1.T==102)
    {
        ...
        if (codee::fill_z(Y,int(o1.N()),double(val)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
