...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets median for each vector in X along dim.\n";
    descr += "\n";
    descr += "Use -d (--dim) to give the dimension (axis) [default=0].\n";
    descr += "Use -d0 to get median along cols.\n";
    descr += "Use -d1 to get median along rows.\n";
    descr += "Use -d2 to get median along slices.\n";
    descr += "Use -d3 to get median along hyperslices.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ median X -o Y \n";
    descr += "$ median X > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        //if (codee::median_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
        if (codee::median_inplace_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
    ...
    else if (i1.T==2u)
    {
        ...
        //if (codee::median_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
        if (codee::median_inplace_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
