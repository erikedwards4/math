...
int main(int argc, char *argv[])
{
    ...
    descr += "Ceils each element of X (rounds toward Inf).\n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::ceil_inplace_s(X,i1.N()))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::ceil_inplace_d(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::ceil_inplace_c(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::ceil_inplace_z(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
