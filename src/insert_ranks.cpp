...
int main(int argc, char *argv[])
{
    ...
    descr += "See insert_sorti for the inverse (indices to sorted X to recover X).\n";
    descr += "\n";
    descr += "This uses (my own implementation of) the insertion sort algorithm.\n";
    descr += "This is faster than qranks if the vectors are short (e.g., L < 128).\n";
    descr += "\n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::insert_ranks_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::insert_ranks_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::insert_ranks_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::insert_ranks_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,a))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
