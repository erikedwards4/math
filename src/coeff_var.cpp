...
int main(int argc, char *argv[])
{
    ...
    descr += "Use -d1 to work along rows.\n";
    descr += "Use -d2 to work along slices.\n";
    descr += "Use -d3 to work along hyperslices.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ coeff_var X -o Y \n";
    descr += "$ coeff_var X > Y \n";
    descr += "$ coeff_var -d1 X > Y \n";
    descr += "$ cat X | coeff_var > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::coeff_var_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,b))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::coeff_var_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,b))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
