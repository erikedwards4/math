...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Normalizes each vector in X along dim to have unit normp.\n";
    descr += "That is, the Lp norm is divided out for each vector in X.\n";
    descr += "\n";
    ...
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ normalizep X -o Y \n";
    descr += "$ normalizep -d1 X > Y \n";
    descr += "$ cat X | normalizep > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::normalizep_inplace_s(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,float(p)))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::normalizep_inplace_d(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,double(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::normalizep_inplace_c(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,float(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::normalizep_inplace_z(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,double(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
