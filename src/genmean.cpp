...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets generalized mean for each vector in X along dim.\n";
    descr += "\n";
    descr += "This is like the Lp norm, except uses mean instead of sum:\n";
    descr += "For each vector, y = mean(|x|^p)^1/p. \n";
    descr += "\n";
    ...
    descr += "$ genmean X -o Y \n";
    descr += "$ genmean X > Y \n";
    descr += "$ genmean -d1 X > Y \n";
    descr += "$ cat X | genmean > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::genmean_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,float(p)))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::genmean_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,double(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::genmean_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,float(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::genmean_z(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,double(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
