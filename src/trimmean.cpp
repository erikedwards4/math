...
int main(int argc, char *argv[])
{
    ...
    if (i1.T==1u)
    {
        ...
        //if (codee::trimmean_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,float(p),float(q)))
    ...
    else if (i1.T==2)
    {
        ...
        //if (codee::trimmean_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,double(p),double(q)))
        if (codee::trimmean_inplace_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,double(p),double(q)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
