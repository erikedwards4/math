...
int main(int argc, char *argv[])
{
    ...
    descr += "For each element of X, does x++ (increment by 1).\n";
    descr += "This operates in-place.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ plusplus X -o Y \n";
    descr += "$ plusplus X > Y \n";
    descr += "$ cat X | plusplus > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (codee::plusplus_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::plusplus_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
