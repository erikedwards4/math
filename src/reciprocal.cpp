...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets reciprocal (1/x) of each element of X.\n";
    descr += "For each element: y = x^-1 = 1/x.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ reciprocal X -o Y \n";
    descr += "$ reciprocal X > Y \n";
    descr += "$ cat X | reciprocal > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::reciprocal_inplace_s(X,i1.N()))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::reciprocal_inplace_d(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::reciprocal_inplace_c(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::reciprocal_inplace_z(X,i1.N()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
