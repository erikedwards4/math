...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Cubes each element of X.\n";
    descr += "For complex X, output is real-valued: y = |x|^3.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ cube X -o Y \n";
    descr += "$ cube X > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (codee::cube_inplace_s(X,int(i1.N())))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::cube_inplace_d(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (codee::cube_inplace_c(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (codee::cube_inplace_z(X,int(i1.N())))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
