...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Zeros the median of each vector in X.\n";
    descr += "For each vector, the median is estimated and then subtracted.\n";
    descr += "\n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::med0_inplace_s(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::med0_inplace_d(X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim)) { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
            catch (...) { cerr << progstr+": " << __LINE__ << errstr << "problem writing output file (Y)" << endl; return 1; }
