...
int main(int argc, char *argv[])
{
    ...
    descr += "Gets index of the median for each vector in X along dim.\n";
    descr += "This is the index with the value closest to the median.\n";
    descr += "For even-length vecs, this takes the higher of the 2 indices.\n";
    descr += "The output is an integer, but data type is float or double.\n";
    descr += "\n";
    descr += "For complex X, this is the min of absolute values.\n";
    descr += "\n";
    ...
    if (i1.T==1)
    {
        ...
        if (codee::imed_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::imed_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
