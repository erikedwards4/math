...
int main(int argc, char *argv[])
{
    ...
    descr += "Raises each element of X to the power of P.\n";
    descr += "\n";
    descr += "X and P must have the same size or broadcast-compatible sizes.\n";
    ...
    if (i1.T==1)
    {
        ...
        if (i1.N()==o1.N())
        {
            if (codee::pow_inplace_s(X,P,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
        ...
        else
        {
            ...
            if (codee::pow_s(Y,X,P,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
    ...
    else if (i1.T==2)
    {
        ...
        if (i1.N()==o1.N())
        {
            if (codee::pow_inplace_d(X,P,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
                try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
        ...
        else
        {
            ...
            if (codee::pow_d(Y,X,P,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
                try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (i1.N()==o1.N())
        {
            if (codee::pow_inplace_c(X,P,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
                try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
        ...
        else
        {
            ...
            if (codee::pow_c(Y,X,P,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
                try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (i1.N()==o1.N())
        {
            if (codee::pow_inplace_z(X,P,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
                try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
        ...
        else
        {
            ...
            if (codee::pow_z(Y,X,P,int(i1.R),int(i1.C),int(i1.S),int(i1.H),int(i2.R),int(i2.C),int(i2.S),int(i2.H),o1.iscolmajor()))
            { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
            if (wo1)
            {
                try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
