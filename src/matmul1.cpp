...
int main(int argc, char *argv[])
{
    ...
    descr += "Matrix multiplication of square matrix X by itself:\n";
    descr += "Y = X*X. \n";
    descr += "Thus, output Y has same size as X.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ matmul1 X -o Y \n";
    descr += "$ matmul1 X > Y \n";
    descr += "$ cat X1 | matmul1 > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        if (codee::matmul1_s(Y,X,i1.R,i1.C,o1.iscolmajor()))
    ...
    else if (i1.T==2u)
    {
        ...
        if (codee::matmul1_d(Y,X,i1.R,i1.C,o1.iscolmajor()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101u)
    {
        ...
        if (codee::matmul1_c(Y,X,i1.R,i1.C,o1.iscolmajor()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102u)
    {
        ...
        if (codee::matmul1_z(Y,X,i1.R,i1.C,o1.iscolmajor()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
