...
int main(int argc, char *argv[])
{
    ...
    descr += "$ prctile -p10 X > Y \n";
    descr += "$ prctile -p90 -d1 X > Y \n";
    descr += "$ cat X | prctile -p25 > Y \n";
    ...
    if (i1.T==1u)
    {
        ...
        //if (codee::prctile_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,float(p)))
        if (codee::prctile_inplace_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,float(p)))
    ...
    else if (i1.T==2)
    {
        ...
        //if (codee::prctile_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,double(p)))
        if (codee::prctile_inplace_d(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim,double(p)))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
