...
int main(int argc, char *argv[])
{
    ...
    //Description
    string descr;
    descr += "Gets mean along dim of X.\n";
    descr += "\n";
    descr += "Use -d (--dim) to give the dimension (axis) [default=0].\n";
    descr += "Use -d0 to get mean along cols.\n";
    descr += "Use -d1 to get mean along rows.\n";
    descr += "Use -d2 to get mean along slices.\n";
    descr += "Use -d3 to get mean along hyperslices.\n";
    descr += "\n";
    descr += "Examples:\n";
    descr += "$ mean X -o Y \n";
    descr += "$ mean X > Y \n";
    ...
    if (i1.T==1)
    {
        ...
        if (codee::mean_s(Y,X,int(i1.R),int(i1.C),int(i1.S),int(i1.H),dim,i1.iscolmajor()))
    ...
    else if (i1.T==2)
    {
        ...
        if (codee::mean_d(Y,X,int(i1.R),int(i1.C),int(i1.S),int(i1.H),dim,i1.iscolmajor()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==101)
    {
        ...
        if (codee::mean_c(Y,X,int(i1.R),int(i1.C),int(i1.S),int(i1.H),dim,i1.iscolmajor()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
    ...
    else if (i1.T==102)
    {
        ...
        if (codee::mean_z(Y,X,int(i1.R),int(i1.C),int(i1.S),int(i1.H),dim,i1.iscolmajor()))
        { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
        if (wo1)
        {
            try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
