...
//Description
string descr;
descr += "Gets sum of absolute-values (L1 norm) along dim of X.\n";
descr += "\n";
...
if (i1.T==1)
{
    ...
    if (codee::asum_s(Y,X,int(i1.R),int(i1.C),int(i1.S),int(i1.H),dim,i1.iscolmajor()))
...
else if (i1.T==101)
{
    ...
    if (codee::asum_c(Y,X,int(i1.R),int(i1.C),int(i1.S),int(i1.H),dim,i1.iscolmajor()))
    { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
        try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
