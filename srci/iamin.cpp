...
descr += "Gets index of min of absolute values for each vector in X along dim.\n";
descr += "\n";
...
if (i1.T==1)
{
    ...
    if (codee::iamin_s(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
...
else if (i1.T==101)
{
    ...
    if (codee::iamin_c(Y,X,i1.R,i1.C,i1.S,i1.H,i1.iscolmajor(),dim))
    { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
        try { ofs1.write(reinterpret_cast<char*>(Y),o1.nbytes()); }
