...
if (i1.T==1)
{
    ...
    //if (codee::max_s(Y,X,int(i1.R),int(i1.C),dim,i1.iscolmajor()))
    if (codee::max_inplace_s(X,int(i1.R),int(i1.C),dim,i1.iscolmajor()))
...
else if (i1.T==101)
{
    ...
    //if (codee::max_c(Y,X,int(i1.R),int(i1.C),dim,i1.iscolmajor()))
    if (codee::max_inplace_c(X,int(i1.R),int(i1.C),dim,i1.iscolmajor()))
    { cerr << progstr+": " << __LINE__ << errstr << "problem during function call" << endl; return 1; }
    if (wo1)
    {
        try { ofs1.write(reinterpret_cast<char*>(X),o1.nbytes()); }
