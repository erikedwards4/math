...
int plus_c (float *Y, const float *X1, const float *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (N1==1)
    {
        for (size_t n=0; n<N; ++n, ++X2, ++Y) { *Y = *X1 + *X2; *++Y = *(X1+1) + *++X2; }
    }    
    else if (N2==1)
    {
        for (size_t n=0; n<N; ++n, ++X1, ++Y) { *Y = *X1 + *X2; *++Y = *++X1 + *(X2+1); }
    }
    else if (N1==N2)
    {
        for (size_t n=0; n<2*N; ++n, ++X1, ++X2, ++Y) { *Y = *X1 + *X2; }
    }
    else if (iscolmajor)
    {
        ...
        for (size_t h=0; h<H; ++h, X1+=h1i, X2+=h2i)
        {
            for (size_t s=0; s<S; ++s, X1+=s1i, X2+=s2i)
            {
                for (size_t c=0; c<C; ++c, X1+=c1i, X2+=c2i)
                {
                    for (size_t r=0; r<R; ++r, ++Y, X1+=r1i, X2+=r2i)
                    {
                        *Y = *X1 + *X2;
                        *++Y = *(X1+1) + *(X2+1);
                    }
    ...
    else
    {
        ...
        for (size_t r=0; r<R; ++r, X1+=r1i, X2+=r2i)
        {
            for (size_t c=0; c<C; ++c, X1+=c1i, X2+=c2i)
            {
                for (size_t s=0; s<S; ++s, X1+=s1i, X2+=s2i)
                {
                    for (size_t h=0; h<H; ++h, ++Y, X1+=h1i, X2+=h2i)
                    {
                        *Y = *X1 + *X2;
                        *++Y = *(X1+1) + *(X2+1);
                    }
...
int plus_inplace_c (float *X1, const float *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (N2==1)
    {
        for (size_t n=0; n<N; ++n, ++X1) { *X1 += *X2; *++X1 += *(X2+1); }
    }
    else if (N==N2)
    {
