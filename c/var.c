...
int var_s (float *Y, float *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const char biased)
{
    ...
    else if (L==N)
    {
        ...
        *Y = cblas_sdot((int)L,X,1,X,1) * den;
    }
    else
    {
        ...
        if (K==1 && (G==1 || B==1))
        {
            ...
            else
            {
                ...
                for (size_t v=0; v<V; v++, X+=L) { *Y++ = cblas_sdot((int)L,X,1,X,1) * den; }
                free(x1); free(xni);
            }
        }
        else if (G==1)
        {
            ...
            cblas_sgemv(CblasRowMajor,CblasTrans,(int)L,(int)V,den,X,(int)V,x1,1,z,Y,1);
            free(x1); free(xni);
        }
        else
        {
            ...
            for (size_t g=0; g<G; g++, X+=B*(L-1))
            {
                for (size_t b=0; b<B; b++, X++)
                {
                    ...
                    *Y++ = cblas_sdot((int)L,x1,1,x1,1) * den;
                }
            }
            free(x1);
        }
...
int var_d (double *Y, double *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const char biased)
{
    ...
    else if (L==N)
    {
        ...
        *Y = cblas_ddot((int)L,X,1,X,1) * den;
    }
    else
    {
        const size_t K = (iscolmajor) ? ((dim==0) ? 1 : (dim==1) ? R : (dim==2) ? R*C : R*C*S) : ((dim==0) ? C*S*H : (dim==1) ? S*H : (dim==2) ? H : 1);
        ...
        if (K==1 && (G==1 || B==1))
        {
            ...
            else
            {
                ...
                for (size_t v=0; v<V; v++, X+=L) { *Y++ = cblas_ddot((int)L,X,1,X,1) * den; }
                free(x1); free(xni);
            }
        }
        else if (G==1)
        {
            ...
            for (size_t n=0; n<N; n++) { X[n] *= X[n]; }
            cblas_dgemv(CblasRowMajor,CblasTrans,(int)L,(int)V,den,X,(int)V,x1,1,z,Y,1);
            free(x1); free(xni);
        }
        else
        {
            ...
            for (size_t g=0; g<G; g++, X+=B*(L-1))
            {
                for (size_t b=0; b<B; b++, X++)
                {
                    ...
                    *Y++ = cblas_ddot((int)L,x1,1,x1,1) * den;
                }
            }
            free(x1);
        }
...
int var_c (float *Y, float *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const char biased)
{
    ...
    else
    {
        ...
        {
            float mn[2], *xni;
            if (!(xni=(float *)malloc(2*L*sizeof(float)))) { fprintf(stderr,"error in var_c: problem with malloc. "); perror("malloc"); return 1; }
...
int var_z (double *Y, double *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const char biased)
{
    ...
    else
    {
        ...
        {
            double mn[2], *xni;
            if (!(xni=(double *)malloc(2*L*sizeof(double)))) { fprintf(stderr,"error in var_z: problem with malloc. "); perror("malloc"); return 1; }
