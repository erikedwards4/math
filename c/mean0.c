...
int mean0_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim)
{
    ...
    else if (L==N)
    {
        ...
        mn /= (float)L; X -= L;
        for (size_t l=L; l>0u; --l, ++X, ++Y) { *Y = *X - mn; }
    }
    else
    {
        ...
        else
        {
            for (size_t g=G; g>0u; --g, X+=B*(L-1u), Y+=B*(L-1u))
            {
                for (size_t b=B; b>0u; --b, X-=K*L-1u, Y-=K*L-1u)
                {
                    ...
                    mn /= (float)L; X -= K*L;
                    for (size_t l=L; l>0u; --l, X+=K, Y+=K) { *Y = *X - mn; }
                }
            }
        }
    }
...
int mean0_c (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim)
{
    ...
    else if (L==N)
    {
        ...
        for (size_t l=L; l>0u; --l, ++X, ++Y) { *Y = *X - mnr; *++Y = *++X - mni; }
    }
    else
    {
        const size_t K = (iscolmajor) ? ((dim==0u) ? 1u : (dim==1u) ? R : (dim==2u) ? R*C : R*C*S) : ((dim==0u) ? C*S*H : (dim==1u) ? S*H : (dim==2u) ? H : 1u);
        ...
        else
        {
            for (size_t g=G; g>0u; --g, X+=2u*B*(L-1u), Y+=2u*B*(L-1u))
            {
                for (size_t b=B; b>0u; --b, X-=2u*K*L-2u, Y-=2u*K*L-2u)
                {
                    ...
                    mnr /= (float)L; mni /= (float)L; X -= 2u*K*L;
                    for (size_t l=L; l>0u; --l, X+=2u*K, Y+=2u*K) { *Y = *X - mni; *++Y = *++X - mnr; }
                }
            }
        }
    }
        ...
        else
        {
            for (size_t g=G; g>0u; --g, X+=2u*B*(L-1u), Y+=2u*B*(L-1u))
            {
                for (size_t b=B; b>0u; --b, X-=2u*K*L-2u, Y-=2u*K*L-2u)
                {
                    mnr = mni = 0.0;
                    for (size_t l=L; l>0u; --l, X+=2u*K-1u) { mnr += *X; mni += *++X; }
                    mnr /= (double)L; mni /= (double)L; X -= 2u*K*L;
                    for (size_t l=L; l>0u; --l, X+=2u*K, Y+=2u*K) { *Y = *X - mni; *++Y = *++X - mnr; }
                }
            }
        }
...
{
    if (dim>3u) { fprintf(stderr,"error in mean0_inplace_s: dim must be in [0 3]\n"); return 1; }
...
{
    if (dim>3u) { fprintf(stderr,"error in mean0_inplace_d: dim must be in [0 3]\n"); return 1; }
...
{
    if (dim>3u) { fprintf(stderr,"error in mean0_inplace_c: dim must be in [0 3]\n"); return 1; }
...
{
    if (dim>3u) { fprintf(stderr,"error in mean0_inplace_z: dim must be in [0 3]\n"); return 1; }
