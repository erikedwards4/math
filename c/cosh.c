...
int cosh_s (float *Y, const float *X, const int N)
{
    ...
    //for (n=0; n<N; n++) { Y[n] = coshf(X[n]); }
    for (n=0; n<N; n++) { xp = expf(X[n]); xm = expf(-X[n]); Y[n] = 0.5f*(xp+xm); }
...
int cosh_d (double *Y, const double *X, const int N)
{
    ...
    //for (n=0; n<N; n++) { Y[n] = cosh(X[n]); }
    for (n=0; n<N; n++) { xp = exp(X[n]); xm = exp(-X[n]); Y[n] = 0.5*(xp+xm); }
...
int cosh_inplace_s (float *X, const int N)
{
    ...
    //for (n=0; n<N; n++) { X[n] = coshf(X[n]); }
    for (n=0; n<N; n++) { xp = expf(X[n]); xm = expf(-X[n]); X[n] = 0.5f*(xp+xm); }
...
int cosh_inplace_d (double *X, const int N)
{
    ...
    //for (n=0; n<N; n++) { X[n] = cosh(X[n]); }
    for (n=0; n<N; n++) { xp = exp(X[n]); xm = exp(-X[n]); X[n] = 0.5*(xp+xm); }
