...
int ctranspose_s (float *Y, const float *X, const int R, const int C, const char iscolmajor)
{
    ...
    {
        for (int c=0, n=0; c<C; c++, n+=R) { cblas_scopy(R,&X[n],1,&Y[c],C); }
    }
    else
    {
        for (int r=0, n=0; r<R; r++, n+=C) { cblas_scopy(C,&X[n],1,&Y[r],R); }
    }
...
int ctranspose_d (double *Y, const double *X, const int R, const int C, const char iscolmajor)
{
    ...
    {
        for (int c=0, n=0; c<C; c++, n+=R) { cblas_dcopy(R,&X[n],1,&Y[c],C); }
    }
    else
    {
        for (int r=0, n=0; r<R; r++, n+=C) { cblas_dcopy(C,&X[n],1,&Y[r],R); }
    }
...
int ctranspose_c (float *Y, const float *X, const int R, const int C, const char iscolmajor)
{
    ...
    {
        for (int c=0, n=0; c<C; c++, n+=2*R) { cblas_ccopy(R,&X[n],1,&Y[2*c],C); }
    }
    else
    {
        for (int r=0, n=0; r<R; r++, n+=2*C) { cblas_ccopy(C,&X[n],1,&Y[2*r],R); }
    }
...
int ctranspose_z (double *Y, const double *X, const int R, const int C, const char iscolmajor)
{
    ...
    {
        for (int c=0, n=0; c<C; c++, n+=2*R) { cblas_zcopy(R,&X[n],1,&Y[2*c],C); }
    }
    else
    {
        for (int r=0, n=0; r<R; r++, n+=2*C) { cblas_zcopy(C,&X[n],1,&Y[2*r],R); }
    }
...
int ctranspose_inplace_s (float *X, const int R, const int C, const char iscolmajor)
{
    ...
    else
    {
        ...
        if (iscolmajor)
        {
            for (int c=0, n=0; c<C; c++, n+=R) { cblas_scopy(R,&Xt[n],1,&X[c],C); }
        }
        else
        {
            for (int r=0, n=0; r<R; r++, n+=C) { cblas_scopy(C,&Xt[n],1,&X[r],R); }
        }
        free(Xt);
    }
    //clock_gettime(CLOCK_REALTIME,&toc);
...
int ctranspose_inplace_d (double *X, const int R, const int C, const char iscolmajor)
{
    ...
    else
    {
        ...
        if (iscolmajor)
        {
            for (int c=0, n=0; c<C; c++, n+=R) { cblas_dcopy(R,&Xt[n],1,&X[c],C); }
        }
        else
        {
            for (int r=0, n=0; r<R; r++, n+=C) { cblas_dcopy(C,&Xt[n],1,&X[r],R); }
        }
        free(Xt);
    }
...
int ctranspose_inplace_c (float *X, const int R, const int C, const char iscolmajor)
{
    ...
    else
    {
        ...
        if (iscolmajor)
        {
            for (int c=0, n=0; c<C; c++, n+=2*R) { cblas_ccopy(R,&Xt[n],1,&X[2*c],C); }
        }
        else
        {
            for (int r=0, n=0; r<R; r++, n+=2*C) { cblas_ccopy(C,&Xt[n],1,&X[2*r],R); }
        }
        free(Xt);
    }
...
int ctranspose_inplace_z (double *X, const int R, const int C, const char iscolmajor)
{
    ...
    else
    {
        ...
        if (iscolmajor)
        {
            for (int c=0, n=0; c<C; c++, n+=2*R) { cblas_zcopy(R,&Xt[n],1,&X[2*c],C); }
        }
        else
        {
            for (int r=0, n=0; r<R; r++, n+=2*C) { cblas_zcopy(C,&Xt[n],1,&X[2*r],R); }
        }
        free(Xt);
    }
