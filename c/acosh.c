...
int acosh_s (float *Y, const float *X, const int N)
{
    ...
    //for (int n=0; n<N; n++) { Y[n] = acoshf(X[n]); }
    for (int n=0; n<N; n++) { Y[n] = logf(X[n] + sqrtf(fmaf(X[n],X[n],-1.0f))); }
...
int acosh_d (double *Y, const double *X, const int N)
{
    ...
    //for (int n=0; n<N; n++) { Y[n] = acosh(X[n]); }
    for (int n=0; n<N; n++) { Y[n] = log(X[n] + sqrt(fma(X[n],X[n],-1.0))); }
...
int acosh_inplace_s (float *X, const int N)
{
    ...
    //for (int n=0; n<N; n++) { X[n] = acoshf(X[n]); }
    for (int n=0; n<N; n++) { X[n] = logf(X[n] + sqrtf(fmaf(X[n],X[n],-1.0f))); }
...
int acosh_inplace_d (double *X, const int N)
{
    ...
    //for (int n=0; n<N; n++) { X[n] = acosh(X[n]); }
    for (int n=0; n<N; n++) { X[n] = log(X[n] + sqrt(fma(X[n],X[n],-1.0))); }
