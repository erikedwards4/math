...
int moments_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const int biased)
{
    ...
    else
    {
        ...
        else if (G==1u)
        {
            ...
            Y += V;
            for (size_t v=V; v>0u; --v, ++sm2, ++sm3, ++sm4, ++Y) { *Y = *sm2 * den2; }
            sm2 -= V; sm3 -= V; sm4 -= V;
            for (size_t v=V; v>0u; --v, ++sm2, ++sm3, ++sm4, ++Y) { *Y = w * *sm3 / (*sm2*sqrtf(*sm2)); }
            sm2 -= V; sm3 -= V; sm4 -= V;
            for (size_t v=V; v>0u; --v, ++sm2, ++sm3, ++sm4, ++Y)
            {
                *Y = (float)Lx * *sm4 / (*sm2**sm2);
                if (!biased) { *Y =  3.0f + (*Y*(float)(Lx+1u)-(float)(3u*(Lx-1u))) * (float)(Lx-1u)/(float)((Lx-2u)*(Lx-3u)); }
            }
...
int moments_d (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const int biased)
{
    ...
    else
    {
        ...
        else if (G==1u)
        {
            ...
            for (size_t v=V; v>0u; --v, ++sm2, ++sm3, ++sm4, ++Y) { *Y = *sm2 * den2; }
            sm2 -= V; sm3 -= V; sm4 -= V;
            for (size_t v=V; v>0u; --v, ++sm2, ++sm3, ++sm4, ++Y) { *Y = w * *sm3 / (*sm2*sqrt(*sm2)); }
            sm2 -= V; sm3 -= V; sm4 -= V;
            for (size_t v=V; v>0u; --v, ++sm2, ++sm3, ++sm4, ++Y)
            {
                *Y = (double)Lx * *sm4 / (*sm2**sm2);
