...
int hypot_c (float *Y, const float *X1, const float *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (N1==1)
    {
        ...
        for (size_t n=0, n2=0; n<N; n++, n2+=2) { Y[n] = sqrtf(x1 + X2[n2]*X2[n2]+X2[n2+1]*X2[n2+1]); }
    }    
    else if (N2==1)
    {
        const float x2 = X2[0]*X2[0] + X2[1]*X2[1];
        for (size_t n=0, n2=0; n<N; n++, n2+=2) { Y[n] = sqrtf(X1[n2]*X1[n2]+X1[n2+1]*X1[n2+1] + x2); }
    }
    else if (N==N1 && N==N2)
    {
        for (size_t n=0, n2=0; n<N; n++, n2+=2) { Y[n] = sqrtf(X1[n2]*X1[n2]+X1[n2+1]*X1[n2+1] + X2[n2]*X2[n2]+X2[n2+1]*X2[n2+1]); }
    }
    else if (iscolmajor)
    {
...
int hypot_z (double *Y, const double *X1, const double *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (N1==1)
    {
        ...
        for (size_t n=0, n2=0; n<N; n++, n2+=2) { Y[n] = sqrt(x1 + X2[n2]*X2[n2]+X2[n2+1]*X2[n2+1]); }
    }    
    else if (N2==1)
    {
        const double x2 = X2[0]*X2[0] + X2[1]*X2[1];
        for (size_t n=0, n2=0; n<N; n++, n2+=2) { Y[n] = sqrt(X1[n2]*X1[n2]+X1[n2+1]*X1[n2+1] + x2); }
    }
    else if (N==N1 && N==N2)
    {
        for (size_t n=0, n2=0; n<N; n++, n2+=2) { Y[n] = sqrt(X1[n2]*X1[n2]+X1[n2+1]*X1[n2+1] + X2[n2]*X2[n2]+X2[n2+1]*X2[n2+1]); }
    }
    else if (iscolmajor)
    {
        const size_t RCSH1 = 2*R1*C1*S1*(H1>1), RCSH2 = 2*R2*C2*S2*(H2>1);
    ...
    else if (iscolmajor)
    {
        ...
        for (size_t h=0, n=0, n1=0; h<H; h++)
        {
            for (size_t s=0; s<S; s++)
            {
                ...
                {
                    size_t n2 = h*RCSH2 + s*RCS2 + c*RC2;
                    for (size_t r=0; r<R; r++, n++, n1+=2, n2+=r2i)
                    {
    ...
    else
    {
        ...
        for (size_t r=0, n=0, n1=0; r<R; r++)
        {
            for (size_t c=0; c<C; c++)
            {
                ...
                {
                    size_t n2 = r*HSCR2 + c*HSC2 + s*HS2;
                    for (size_t h=0; h<H; h++, n++, n1+=2, n2+=h2i)
                    {
                        X1[n] = sqrtf(X1[n1]*X1[n1]+X1[n1+1]*X1[n1+1] + X2[n2]*X2[n2]+X2[n2+1]*X2[n2+1]);
                    }
                }
