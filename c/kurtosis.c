...
int kurtosis_s (float *Y, float *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim, const char biased)
{
    ...
    else if (L==N)
    {
        ...
        mn *= den;
        for (size_t l=0; l<L; ++l) { x = *--X - mn; x2 = x*x; sm2 += x2; sm4 += x2*x2; }
        *Y = L * sm4 / (sm2*sm2);
    ...
    else
    {
        ...
        if (K==1 && (G==1 || B==1))
        {
            ...
            for (size_t v=0; v<V; ++v, ++Y)
            {
                ...
                for (size_t l=0; l<L; ++l, ++X) { mn += *X; }
                mn *= den; X -= L;
                for (size_t l=0; l<L; ++l, ++X) { x = *X - mn; x2 = x*x; sm2 += x2; sm4 += x2*x2; }
                *Y = L * sm4 / (sm2*sm2);
                if (!biased) { *Y =  3.0f + (*Y*(L+1)-3*(L-1)) * (L-1)/((L-2)*(L-3)); }
            }
        }
        ...
        else
        {
            ...
            for (size_t g=0; g<G; ++g, X+=B*(L-1))
            {
                for (size_t b=0; b<B; ++b, ++X, ++Y)
                {
                    ...
                    mn += *X; mn *= den;
                    for (size_t l=0; l<L-1; ++l, X-=K) { x = *X - mn; x2 = x*x; sm2 += x2; sm4 += x2*x2; }
