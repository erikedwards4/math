...
{
    if (dim>3u) { fprintf(stderr,"error in idr1_s: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (L<2u) { fprintf(stderr,"error in idr1_s: L (vec length) must be > 1\n"); return 1; }
    float mn, mx, rng;
    ...
    else if (L==N)
    {
        ...
        Y -= L; X -= L;
        if (LAPACKE_slasrt_work('I',(int)L,Y)) { fprintf(stderr,"error in idr1_s: problem with LAPACKE function\n"); }
        Y += i1;
        mn = w1**Y + w2**(Y+1);
        Y += i2 - i1;
        mx = w3**Y + w4**(Y+1);
...
{
    if (dim>3u) { fprintf(stderr,"error in idr1_d: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (L<2u) { fprintf(stderr,"error in idr1_d: L (vec length) must be > 1\n"); return 1; }
    double mn, mx, rng;
    ...
    else if (L==N)
    {
        for (size_t l=L; l>0u; --l, ++X, ++Y) { *Y = *X; }
        Y -= L; X -= L;
        if (LAPACKE_dlasrt_work('I',(int)L,Y)) { fprintf(stderr,"error in idr1_d: problem with LAPACKE function\n"); }
        Y += i1;
        mn = w1**Y + w2**(Y+1);
        Y += i2 - i1;
        mx = w3**Y + w4**(Y+1);
...
{
    if (dim>3u) { fprintf(stderr,"error in idr1_inplace_s: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (L<2u) { fprintf(stderr,"error in idr1_inplace_s: L (vec length) must be > 1\n"); return 1; }
    float mn, mx, rng;
    ...
    float *X1;
    if (!(X1=(float *)malloc(L*sizeof(float)))) { fprintf(stderr,"error in idr1_inplace_s: problem with malloc. "); perror("malloc"); return 1; }
    ...
    else if (L==N)
    {
        ...
        X1 -= L;
        if (LAPACKE_slasrt_work('I',(int)L,X1)) { fprintf(stderr,"error in idr1_inplace_s: problem with LAPACKE function\n"); }
        X1 += i1;
        mn = w1**X1 + w2**(X1+1);
        X1 += i2 - i1;
        mx = w3**X1 + w4**(X1+1);
...
{
    if (dim>3u) { fprintf(stderr,"error in idr1_inplace_d: dim must be in [0 3]\n"); return 1; }

    const size_t N = R*C*S*H;
    const size_t L = (dim==0u) ? R : (dim==1u) ? C : (dim==2u) ? S : H;
    if (L<2u) { fprintf(stderr,"error in idr1_inplace_d: L (vec length) must be > 1\n"); return 1; }
    double mn, mx, rng;
    ...
    double *X1;
    if (!(X1=(double *)malloc(L*sizeof(double)))) { fprintf(stderr,"error in idr1_inplace_d: problem with malloc. "); perror("malloc"); return 1; }
    ...
    else if (L==N)
    {
        for (size_t l=L; l>0u; --l, ++X, ++X1) { *X1 = *X; }
        X1 -= L;
        if (LAPACKE_dlasrt_work('I',(int)L,X1)) { fprintf(stderr,"error in idr1_inplace_d: problem with LAPACKE function\n"); }
        X1 += i1;
        mn = w1**X1 + w2**(X1+1);
        X1 += i2 - i1;
        mx = w3**X1 + w4**(X1+1);
