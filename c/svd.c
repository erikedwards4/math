...
int svd_s (float *U, float *S, float *Vt, const float *X, const size_t R, const size_t C, const char iscolmajor, const size_t K)
{
    ...
    else
    {
        ...
        for (size_t n=0; n<N; ++n, ++X, ++Xtmp) { *Xtmp = *X; }
        Xtmp -= N;
        ...
        if (K>0 && K<Kmax)
        {
            if (iscolmajor)
            {
                ...
                {
                    for (size_t r=0; r<K; ++r, ++Vt) { *Vt = *(Vt+c*(Kmax-K)); }
                }
            }
            else
            {
                ...
                {
                    for (size_t c=0; c<K; ++c, ++U) { *U = *(U+r*(Kmax-K)); }
                }
            }
        }
...
int svd_c (float *U, float *S, float *Vt, const float *X, const size_t R, const size_t C, const char iscolmajor, const size_t K)
{
    ...
    else
    {
        ...
        for (size_t n=0; n<2*N; ++n, ++X, ++Xtmp) { *Xtmp = *X; }
        Xtmp -= 2*N;
        ...
        if (K>0 && K<Kmax)
        {
            if (iscolmajor)
            {
                for (size_t c=0; c<C; ++c)
                {
                    for (size_t r=0; r<K; ++r, ++Vt) { *Vt = *(Vt+2*c*(Kmax-K)); ++Vt; *Vt = *(Vt+2*c*(Kmax-K)); }
                }
            }
            else
            {
                for (size_t r=0; r<R; ++r)
                {
                    for (size_t c=0; c<K; ++c, ++U) { *U = *(U+2*r*(Kmax-K)); ++U; *U = *(U+2*r*(Kmax-K)); }
                }
            }
        }
