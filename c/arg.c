...
{
    //for (size_t n=N; n>0u; --n, ++X, ++Y) { *Y = atan2f(0.0f,*X); }
    for (size_t n=N; n>0u; --n, ++X, ++Y) { *Y = (float)(*X<0.0f) * (float)M_PI; }
...
{
    for (size_t n=N; n>0u; --n, ++X, ++Y) { *Y = (double)(*X<0.0) * M_PI; }
...
{
    for (size_t n=N; n>0u; --n, X+=2, ++Y) { *Y = atan2f(*(X+1),*X); }
...
{
    for (size_t n=N; n>0u; --n, X+=2, ++Y) { *Y = atan2(*(X+1),*X); }
...
{
    for (size_t n=N; n>0u; --n, ++X) { *X = (float)(*X<0.0f) * (float)M_PI; }
    //for (size_t n=N; n>0u; --n) { *X = (*X>0.0f)*(float)M_PI; }
...
{
    for (size_t n=N; n>0u; --n, ++X) { *X = (double)(*X<0.0) * M_PI; }
