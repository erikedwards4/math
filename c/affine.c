...
int affine_s (float *Y, const float *X, const float *A, const float *B, const size_t R, const size_t C, const size_t S, const size_t H, const size_t Ly, const char iscolmajor, const size_t dim)
{
    ...
    else if (Lx==N)
    {
        if (Na<30000u)
        {
            for (size_t ly=0u; ly<Ly; ++ly, X-=Lx, ++B, ++Y)
            {
                sm2 = *B;
        ...
        {
            for (size_t ly=0u; ly<Ly; ++ly, ++B, ++Y) { *Y = *B; }
            Y -= Ly;
    ...
    else
    {
        ...
        {
            for (size_t g=0u; g<G; ++g, X+=BS*(Lx-1u), Y+=BS*(Ly-1u))
            {
                for (size_t b=0u; b<BS; ++b, ++X, A-=Lx*Ly, B-=Ly, Y-=K*Ly-1u)
                {
                    for (size_t ly=0u; ly<Ly; ++ly, X-=K*Lx, ++B, Y+=K)
                    {
                        sm2 = *B;
...
int affine_c (float *Y, const float *X, const float *A, const float *B, const size_t R, const size_t C, const size_t S, const size_t H, const size_t Ly, const char iscolmajor, const size_t dim)
{
    ...
    else if (Lx==N)
    {
        if (Lx<30000)
        {
            for (size_t ly=0u; ly<Ly; ++ly, X-=2u*Lx, ++B, ++Y)
            {
                sm2r = *B; sm2i = *++B;
        ...
        else
        {
            ...
            for (size_t ly=0u; ly<2u*Ly; ++ly, ++B, ++Y) { *Y = *B; }
            Y -= 2u*Ly;
    ...
    else
    {
        ...
        if (K==1u && (G==1u || BS==1u))
        {
            if (Na<4500u)
            {
                for (size_t v=0u; v<V; ++v, A-=2u*Na, B-=2u*Ly)
                {
                    for (size_t ly=0u; ly<Ly; ++ly, ++B, ++Y)
                    {
                        sm2r = *B; sm2i = *++B;
            ...
            {
                const float o[2] = {1.0f,0.0f};
                for (size_t v=0u; v<V; ++v, X+=2u*Lx, B-=2u*Ly, Y+=2u*Ly)
                {
                    for (size_t ly=0u; ly<2u*Ly; ++ly, ++B, ++Y) { *Y = *B; }
                    Y -= 2u*Ly;
                    cblas_cgemv(CblasColMajor,CblasTrans,(int)Lx,(int)Ly,o,A,(int)Lx,X,1,o,Y,1);
        ...
        {
            for (size_t g=0u; g<G; ++g, X+=2u*BS*(Lx-1u), Y+=2u*BS*(Ly-1u))
            {
                for (size_t b=0u; b<BS; ++b, X+=2, A-=2u*Na, B-=2u*Ly, Y-=2u*K*Ly-2u)
                {
                    for (size_t ly=0u; ly<Ly; ++ly, X-=2u*K*Lx, ++B, Y+=2u*K-1u)
                    {
                        sm2r = *B; sm2i = *++B;
