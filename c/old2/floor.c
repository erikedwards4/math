...
int floor_c (float *Y, const float *X, const int N)
{
    ...
    for (n=0; n<2*N; n++) { Y[n] = floorf(X[n]); }
...
int floor_inplace_c (float *X, const int N)
{
    ...
    for (n=0; n<2*N; n++) { X[n] = floorf(X[n]); }
