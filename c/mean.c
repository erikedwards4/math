...
int mean_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim)
{
    ...
    else if (SH==1)
    {
        ...
        float *xni;
        if (!(xni=(float *)malloc(N1*sizeof(float)))) { fprintf(stderr,"error in mean_s: problem with malloc. "); perror("malloc"); return 1; }
        cblas_scopy((int)N1,&ni,0,xni,1);
    ...
    else if (iscolmajor && dim<2)
    {
        ...
        {
            for (size_t s=0; s<S; s++, X+=RC, Y+=RC/N1)
            {
...
int mean_d (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim)
{
    ...
    else if (SH==1)
    {
        ...
        const int lda = (iscolmajor) ? (int)R : (int)C;
        double *xni;
        if (!(xni=(double *)malloc(N1*sizeof(double)))) { fprintf(stderr,"error in mean_d: problem with malloc. "); perror("malloc"); return 1; }
        cblas_dcopy((int)N1,&ni,0,xni,1);
...
int mean_c (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim)
{
    ...
    else if (SH==1)
    {
        ...
        float *xni;
        if (!(xni=(float *)malloc(2*N1*sizeof(float)))) { fprintf(stderr,"error in mean_c: problem with malloc. "); perror("malloc"); return 1; }
        cblas_ccopy((int)N1,ni,0,xni,1);
    ...
    else if (iscolmajor && dim<2)
    {
        ...
        float *xni;
        if (!(xni=(float *)malloc(2*N1*sizeof(float)))) { fprintf(stderr,"error in mean_c: problem with malloc. "); perror("malloc"); return 1; }
        cblas_ccopy((int)N1,o,0,xni,1);
        for (size_t h=0; h<H; h++)
        {
            for (size_t s=0; s<S; s++, X+=2*RC, Y+=2*RC/N1)
            {
    ...
    else
    {
        ...
        {
            for (size_t m=0; m<M; m++, X+=2*J, Y+=2)
            {
                cblas_cdotu_sub((int)N1,X,(int)K,ni,0,(_Complex float *)Y);
            }
        }
    }
...
int mean_z (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const char iscolmajor, const size_t dim)
{
    ...
    else if (SH==1)
    {
        ...
        double *xni;
        if (!(xni=(double *)malloc(2*N1*sizeof(double)))) { fprintf(stderr,"error in mean_z: problem with malloc. "); perror("malloc"); return 1; }
        cblas_zcopy((int)N1,ni,0,xni,1);
    ...
    else
    {
        ...
        for (size_t l=0; l<L; l++, X+=2*M*(N1-J))
        {
            for (size_t m=0; m<M; m++, X+=2*J, Y+=2)
            {
                cblas_zdotu_sub((int)N1,X,(int)K,ni,0,(_Complex double *)Y);
            }
        }
    }
