...
int mean_s (float *Y, const float *X, const int R, const int C, const int S, const int H, const int dim, const char iscolmajor)
{
    ...
    else if (SH==1)
    {
        ...
        if (!(x1=(float *)malloc((size_t)N1*sizeof(float)))) { fprintf(stderr,"error in mean_s: problem with malloc. "); perror("malloc"); return 1; }
    ...
    // else
    // {
    //     while (n<N) { Y[n/N] += X[n]; n++; }
    //     cblas_sscal(N/N1,ni,Y,1);
    // }
...
int mean_d (double *Y, const double *X, const int R, const int C, const int S, const int H, const int dim, const char iscolmajor)
{
    ...
    else if (SH==1)
    {
        ...
        const CBLAS_ORDER Ord = (iscolmajor) ? CblasColMajor : CblasRowMajor;
        const int lda = (iscolmajor) ? R : C;
        if (!(x1=(double *)malloc((size_t)N1*sizeof(double)))) { fprintf(stderr,"error in mean_d: problem with malloc. "); perror("malloc"); return 1; }
...
int mean_c (float *Y, const float *X, const int R, const int C, const int S, const int H, const int dim, const char iscolmajor)
{
    ...
    else if (SH==1)
    {
        ...
        const CBLAS_ORDER Ord = (iscolmajor) ? CblasColMajor : CblasRowMajor;
        const int lda = (iscolmajor) ? R : C;
        if (!(x1=(float *)malloc((size_t)(2*N1)*sizeof(float)))) { fprintf(stderr,"error in mean_c: problem with malloc. "); perror("malloc"); return 1; }
        cblas_ccopy(N1,ni,0,x1,1);
    ...
    else
    {
        ...
        for (l=0; l<L; l++)
        {
            ...
            {
                cblas_cdotu_sub(N1,&X[n],K,ni,0,(openblas_complex_float *)(&Y[n2]));
            }
        }
    }
...
int mean_z (double *Y, const double *X, const int R, const int C, const int S, const int H, const int dim, const char iscolmajor)
{
    ...
    else if (SH==1)
    {
        ...
        const CBLAS_ORDER Ord = (iscolmajor) ? CblasColMajor : CblasRowMajor;
        const int lda = (iscolmajor) ? R : C;
        if (!(x1=(double *)malloc((size_t)(2*N1)*sizeof(double)))) { fprintf(stderr,"error in mean_z: problem with malloc. "); perror("malloc"); return 1; }
        cblas_zcopy(N1,ni,0,x1,1);
    ...
    else
    {
        ...
        for (l=0; l<L; l++)
        {
            for (m=0, n=2*l*M*N1; m<M; m++, n+=2*J, n2+=2)
            {
                cblas_zdotu_sub(N1,&X[n],K,ni,0,(openblas_complex_double *)(&Y[n2]));
            }
        }
    }
