...
int repmat_s (float *Y, const float *X, const int R, const int C, const int S, const int H, const int Nr, const int Nc, const int Ns, const int Nh, const char iscolmajor)
{
    ...
    if (iscolmajor)
    {
        if (R*Nr==1)
        {
            for (h=0; h<H*Nh; h++)
            {
                ...
                {
                    n = SCR*h%H + CR*s%S;
                    for (c=0; c<Nc; c++, n2+=C) { cblas_scopy(C,&X[n],1,&Y[n2],1); }
                }
            }
        }
        else
            ...
            for (h=0; h<H*Nh; h++)
            {
                for (s=0; s<S*Ns; s++)
                {
                    for (c=0; c<C*Nc; c++)
                    {
                        ...
                        for (r=0; r<Nr; r++, n2+=R) { cblas_scopy(R,&X[n],1,&Y[n2],1); }
                    }
                }
            }
        }
    ...
    else
    {
        if (CSH*Nc*Ns*Nh==1)
        {
            for (r=0; r<Nr; r++, n2+=R) { cblas_scopy(R,X,1,&Y[n2],1); }
        }
        else if (SH*Ns*Nh==1)
        {
            for (r=0; r<R*Nr; r++)
            ...
            for (r=0; r<R*Nr; r++)
            {
                for (c=0; c<C*Nc; c++)
                {
                    for (s=0; s<S*Ns; s++)
                    {
                        ...
                        for (h=0; h<Nh; h++, n2+=H) { cblas_scopy(H,&X[n],1,&Y[n2],1); }
                    }
                }
            }
        }
...
int repmat_d (double *Y, const double *X, const int R, const int C, const int S, const int H, const int Nr, const int Nc, const int Ns, const int Nh, const char iscolmajor)
{
    ...
    if (iscolmajor)
    {
        if (R*Nr==1)
        {
            for (h=0; h<H*Nh; h++)
            {
                ...
                {
                    n = SCR*h%H + CR*s%S;
                    for (c=0; c<Nc; c++, n2+=C) { cblas_dcopy(C,&X[n],1,&Y[n2],1); }
                }
            }
        }
        else
        {
            for (h=0; h<H*Nh; h++)
            {
                for (s=0; s<S*Ns; s++)
                {
                    ...
                    {
                        n = SCR*h%H + CR*s%S + R*c%C;
                        for (r=0; r<Nr; r++, n2+=R) { cblas_dcopy(R,&X[n],1,&Y[n2],1); }
                    }
                }
            }
        }
    ...
    else
    {
        if (CSH*Nc*Ns*Nh==1)
        {
            for (r=0; r<Nr; r++, n2+=R) { cblas_dcopy(R,X,1,&Y[n2],1); }
        }
        else if (SH*Ns*Nh==1)
        {
            for (r=0; r<R*Nr; r++)
        ...
        else
        {
            for (r=0; r<R*Nr; r++)
            {
                for (c=0; c<C*Nc; c++)
                {
                    ...
                    {
                        n = CSH*r%R + SH*c%C + H*s%S;
                        for (h=0; h<Nh; h++, n2+=H) { cblas_dcopy(H,&X[n],1,&Y[n2],1); }
                    }
                }
            }
        }
...
int repmat_c (float *Y, const float *X, const int R, const int C, const int S, const int H, const int Nr, const int Nc, const int Ns, const int Nh, const char iscolmajor)
{
    ...
    if (iscolmajor)
    {
        if (R*Nr==1)
        {
            for (h=0; h<H*Nh; h++)
            {
                ...
                {
                    n = 2*(SCR*h%H + CR*s%S);
                    for (c=0; c<Nc; c++, n2+=2*C) { cblas_ccopy(C,&X[n],1,&Y[n2],1); }
                }
            }
        }
        else
            ...
            for (h=0; h<H*Nh; h++)
            {
                for (s=0; s<S*Ns; s++)
                {
                    for (c=0; c<C*Nc; c++)
                    {
                        ...
                        for (r=0; r<Nr; r++, n2+=2*R) { cblas_ccopy(R,&X[n],1,&Y[n2],1); }
                    }
                }
            }
        }
    ...
    else
    {
        if (CSH*Nc*Ns*Nh==1)
        {
            for (r=0; r<Nr; r++, n2+=2*R) { cblas_ccopy(R,X,1,&Y[n2],1); }
        }
        else if (SH*Ns*Nh==1)
        {
            for (r=0; r<R*Nr; r++)
            ...
            for (r=0; r<R*Nr; r++)
            {
                for (c=0; c<C*Nc; c++)
                {
                    for (s=0; s<S*Ns; s++)
                    {
                        ...
                        for (h=0; h<Nh; h++, n2+=2*H) { cblas_ccopy(H,&X[n],1,&Y[n2],1); }
                    }
                }
            }
        }
...
int repmat_z (double *Y, const double *X, const int R, const int C, const int S, const int H, const int Nr, const int Nc, const int Ns, const int Nh, const char iscolmajor)
{
    ...
    if (iscolmajor)
    {
        if (R*Nr==1)
        {
            for (h=0; h<H*Nh; h++)
            {
                ...
                {
                    n = 2*(SCR*h%H + CR*s%S);
                    for (c=0; c<Nc; c++, n2+=2*C) { cblas_zcopy(C,&X[n],1,&Y[n2],1); }
                }
            }
        }
        else
        {
            for (h=0; h<H*Nh; h++)
            {
                for (s=0; s<S*Ns; s++)
                {
                    ...
                    {
                        n = 2*(SCR*h%H + CR*s%S + R*c%C);
                        for (r=0; r<Nr; r++, n2+=2*R) { cblas_zcopy(R,&X[n],1,&Y[n2],1); }
                    }
                }
            }
        }
    ...
    else
    {
        if (CSH*Nc*Ns*Nh==1)
        {
            for (r=0; r<Nr; r++, n2+=2*R) { cblas_zcopy(R,X,1,&Y[n2],1); }
        }
        else if (SH*Ns*Nh==1)
        {
            for (r=0; r<R*Nr; r++)
        ...
        else
        {
            for (r=0; r<R*Nr; r++)
            {
                for (c=0; c<C*Nc; c++)
                {
                    ...
                    {
                        n = 2*(CSH*r%R + SH*c%C + H*s%S);
                        for (h=0; h<Nh; h++, n2+=2*H) { cblas_zcopy(H,&X[n],1,&Y[n2],1); }
                    }
                }
            }
        }
