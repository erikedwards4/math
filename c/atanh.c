...
int atanh_s (float *Y, const float *X, const int N)
{
    ...
    //for (n=0; n<N; n++) { Y[n] = atanhf(X[n]); }
    for (n=0; n<N; n++) { Y[n] = 0.5f * logf((1.0f+X[n])/(1.0f-X[n])); }
...
int atanh_d (double *Y, const double *X, const int N)
{
    ...
    //for (n=0; n<N; n++) { Y[n] = atanh(X[n]); }
    for (n=0; n<N; n++) { Y[n] = 0.5 * log((1.0+X[n])/(1.0-X[n])); }
...
int atanh_inplace_s (float *X, const int N)
{
    ...
    //for (n=0; n<N; n++) { X[n] = atanhf(X[n]); }
    for (n=0; n<N; n++) { X[n] = 0.5f * logf((1.0f+X[n])/(1.0f-X[n])); }
...
int atanh_inplace_d (double *X, const int N)
{
    ...
    //for (n=0; n<N; n++) { X[n] = atanh(X[n]); }
    for (n=0; n<N; n++) { X[n] = 0.5 * log((1.0+X[n])/(1.0-X[n])); }
