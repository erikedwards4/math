...
int atanh_s (float *Y, const float *X, const size_t N)
{
    //for (size_t n=N; n>0u; --n, ++X, ++Y) { *Y = atanhf(*X); }
    for (size_t n=N; n>0u; --n, ++X, ++Y) { *Y = 0.5f * logf((1.0f+*X)/(1.0f-*X)); }
...
int atanh_d (double *Y, const double *X, const size_t N)
{
    //for (size_t n=N; n>0u; --n, ++X, ++Y) { *Y = atanh(*X); }
    for (size_t n=N; n>0u; --n, ++X, ++Y) { *Y = 0.5 * log((1.0+*X)/(1.0-*X)); }
...
int atanh_inplace_s (float *X, const size_t N)
{
    //for (size_t n=N; n>0u; --n, ++X) { *X = atanhf(*X); }
    for (size_t n=N; n>0u; --n, ++X) { *X = 0.5f * logf((1.0f+*X)/(1.0f-*X)); }
...
int atanh_inplace_d (double *X, const size_t N)
{
    //for (size_t n=N; n>0u; --n, ++X) { *X = atanh(*X); }
    for (size_t n=N; n>0u; --n, ++X) { *X = 0.5 * log((1.0+*X)/(1.0-*X)); }
