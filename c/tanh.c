...
int tanh_s (float *Y, const float *X, const int N)
{
    ...
    //for (int n=0; n<N; n++) { Y[n] = tanhf(X[n]); }
    for (int n=0; n<N; n++) { xp = expf(X[n]); xm = expf(-X[n]); Y[n] = (xp-xm)/(xp+xm); }
...
int tanh_d (double *Y, const double *X, const int N)
{
    ...
    //for (int n=0; n<N; n++) { Y[n] = tanh(X[n]); }
    for (int n=0; n<N; n++) { xp = exp(X[n]); xm = exp(-X[n]); Y[n] = (xp-xm)/(xp+xm); }
...
int tanh_inplace_s (float *X, const int N)
{
    ...
    //for (int n=0; n<N; n++) { X[n] = tanhf(X[n]); }
    //for (int n=0; n<N; n++) { x = expf(2.0f*X[n]); X[n] = (x-1.0f)/(x+1.0f); }   //this is faster, but less numerical accuracy
    for (int n=0; n<N; n++) { xp = expf(X[n]); xm = expf(-X[n]); X[n] = (xp-xm)/(xp+xm); }
...
int tanh_inplace_d (double *X, const int N)
{
    ...
    for (int n=0; n<N; n++) { X[n] = tanh(X[n]); }
    for (int n=0; n<N; n++) { xp = exp(X[n]); xm = exp(-X[n]); X[n] = (xp-xm)/(xp+xm); }
