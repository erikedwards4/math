...
int iqr1_s (float *Y, const float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const int m1)
{
    ...
    else if (L==N)
    {
        ...
        Y -= L; X -= L;
        if (LAPACKE_slasrt_work('I',(int)L,Y)) { fprintf(stderr,"error in iqr1_s: problem with LAPACKE function\n"); }
        Y += i1;
        mn = w1**Y + w2**(Y+1);
        Y += i2 - i1;
        mx = w3**Y + w4**(Y+1);
...
int iqr1_d (double *Y, const double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const int m1)
{
    ...
    else if (L==N)
    {
        for (size_t l=L; l>0u; --l, ++X, ++Y) { *Y = *X; }
        Y -= L; X -= L;
        if (LAPACKE_dlasrt_work('I',(int)L,Y)) { fprintf(stderr,"error in iqr1_d: problem with LAPACKE function\n"); }
        Y += i1;
        mn = w1**Y + w2**(Y+1);
        Y += i2 - i1;
        mx = w3**Y + w4**(Y+1);
...
int iqr1_inplace_s (float *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const int m1)
{
    ...
    else if (L==N)
    {
        ...
        X1 -= L;
        if (LAPACKE_slasrt_work('I',(int)L,X1)) { fprintf(stderr,"error in iqr1_inplace_s: problem with LAPACKE function\n"); }
        X1 += i1;
        mn = w1**X1 + w2**(X1+1);
        X1 += i2 - i1;
        mx = w3**X1 + w4**(X1+1);
...
int iqr1_inplace_d (double *X, const size_t R, const size_t C, const size_t S, const size_t H, const int iscolmajor, const size_t dim, const int m1)
{
    ...
    else if (L==N)
    {
        for (size_t l=L; l>0u; --l, ++X, ++X1) { *X1 = *X; }
        X1 -= L;
        if (LAPACKE_dlasrt_work('I',(int)L,X1)) { fprintf(stderr,"error in iqr1_inplace_d: problem with LAPACKE function\n"); }
        X1 += i1;
        mn = w1**X1 + w2**(X1+1);
        X1 += i2 - i1;
        mx = w3**X1 + w4**(X1+1);
