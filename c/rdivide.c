...
int rdivide_s (float *Y, const float *X1, const float *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (H1<0) { fprintf(stderr,"error in rdivide_s: H1 (num hyperslices X1) must be nonnegative\n"); return 1; }
    if (R2<0) { fprintf(stderr,"error in rdivide_s: R2 (num rows X2) must be nonnegative\n"); return 1; }
    if (C2<0) { fprintf(stderr,"error in rdivide_s: C2 (num cols X2) must be nonnegative\n"); return 1; }
...
int rdivide_d (double *Y, const double *X1, const double *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (H1<0) { fprintf(stderr,"error in rdivide_d: H1 (num hyperslices X1) must be nonnegative\n"); return 1; }
    if (R2<0) { fprintf(stderr,"error in rdivide_d: R2 (num rows X2) must be nonnegative\n"); return 1; }
    if (C2<0) { fprintf(stderr,"error in rdivide_d: C2 (num cols X2) must be nonnegative\n"); return 1; }
...
int rdivide_c (float *Y, const float *X1, const float *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (H1<0) { fprintf(stderr,"error in rdivide_c: H1 (num hyperslices X1) must be nonnegative\n"); return 1; }
    if (R2<0) { fprintf(stderr,"error in rdivide_c: R2 (num rows X2) must be nonnegative\n"); return 1; }
    ...
    if (N1==1)
    {
        while (n2<2*N)
        {
            ...
            Y[n2] = (X1[0]*X2[n2]+X1[1]*X2[n2+1]) / uv2;
            Y[n2+1] = (X1[1]*X2[n2]-X1[0]*X2[n2+1]) / uv2;
            n2+=2;
        }
    }    
    else if (N2==1)
    {
        uv2 = X2[0]*X2[0] + X2[1]*X2[1];
        while (n2<2*N)
        {
            Y[n2] = (X1[n2]*X2[0]+X1[n2+1]*X2[1]) / uv2;
            Y[n2+1] = (X1[n2+1]*X2[0]-X1[n2]*X2[1]) / uv2;
            n2+=2;
        }
    }
    else if (N==N1 && N==N2)
    {
        while (n<2*N)
        {
            uv2 = X2[n]*X2[n] + X2[n+1]*X2[n+1];
            Y[n] = (X1[n]*X2[n]+X1[n+1]*X2[n+1]) / uv2;
            Y[n+1] = (X1[n+1]*X2[n]-X1[n]*X2[n+1]) / uv2;
            n+=2;
        }
    }
    else if (iscolmajor)
            ...
            for (size_t s=0; s<S; s++)
            {
                for (size_t c=0; c<C; c++)
                {
                    ...
                    for (size_t r=0; r<R; r++)
                    {
                        ...
                        Y[n+1] = (X1[n1+1]*X2[n2]-X1[n1]*X2[n2+1]) / uv2;
                        n+=2; n1 += r1i; n2 += r2i;
                    }
                }
            }
...
int rdivide_z (double *Y, const double *X1, const double *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (H1<0) { fprintf(stderr,"error in rdivide_z: H1 (num hyperslices X1) must be nonnegative\n"); return 1; }
    if (R2<0) { fprintf(stderr,"error in rdivide_z: R2 (num rows X2) must be nonnegative\n"); return 1; }
...
int rdivide_inplace_s (float *X1, const float *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (H1<0) { fprintf(stderr,"error in rdivide_inplace_s: H1 (num hyperslices X1) must be nonnegative\n"); return 1; }
    if (R2<0) { fprintf(stderr,"error in rdivide_inplace_s: R2 (num rows X2) must be nonnegative\n"); return 1; }
    ...
    if (N2==1)
    {
        //const float sc = 1.0f / X2[0];
        //cblas_sscal(N,sc,X1,1);
        //while (n<N) { X1[n] *= sc; n++; }
...
int rdivide_inplace_d (double *X1, const double *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (H1<0) { fprintf(stderr,"error in rdivide_inplace_d: H1 (num hyperslices X1) must be nonnegative\n"); return 1; }
    if (R2<0) { fprintf(stderr,"error in rdivide_inplace_d: R2 (num rows X2) must be nonnegative\n"); return 1; }
...
int rdivide_inplace_c (float *X1, const float *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (H1<0) { fprintf(stderr,"error in rdivide_inplace_c: H1 (num hyperslices X1) must be nonnegative\n"); return 1; }
    if (R2<0) { fprintf(stderr,"error in rdivide_inplace_c: R2 (num rows X2) must be nonnegative\n"); return 1; }
    ...
    if (N2==1)
    {
        ...
        while (n<2*N)
        {
            tmp = (X1[n]*X2[0]+X1[n+1]*X2[1]) / uv2;
            X1[n+1] = (X1[n+1]*X2[0]-X1[n]*X2[1]) / uv2;
            X1[n] = tmp; n+=2;
        }
    }
    else if (N==N1 && N==N2)
    {
        ...
        {
            uv2 = X2[n]*X2[n] + X2[n+1]*X2[n+1];
            tmp = (X1[n]*X2[n]+X1[n+1]*X2[n+1]) / uv2;
            X1[n+1] = (X1[n+1]*X2[n]-X1[n]*X2[n+1]) / uv2;
            X1[n] = tmp; n+=2;
        }
    }
    else if (iscolmajor)
            ...
            for (size_t s=0; s<S; s++)
            {
                for (size_t c=0; c<C; c++)
                {
                    ...
                    for (size_t r=0; r<R; r++)
                    {
                        ...
                        X1[n+1] = (X1[n+1]*X2[n2]-X1[n]*X2[n2+1]) / uv2;
                        X1[n] = tmp; n+=2; n2 += r2i;
                    }
                }
            }
...
int rdivide_inplace_z (double *X1, const double *X2, const size_t R1, const size_t C1, const size_t S1, const size_t H1, const size_t R2, const size_t C2, const size_t S2, const size_t H2, const char iscolmajor)
{
    ...
    if (H1<0) { fprintf(stderr,"error in rdivide_inplace_z: H1 (num hyperslices X1) must be nonnegative\n"); return 1; }
    if (R2<0) { fprintf(stderr,"error in rdivide_inplace_z: R2 (num rows X2) must be nonnegative\n"); return 1; }
